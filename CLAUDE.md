...
**Task() — БЛОКИРУЮЩИЙ вызов.** PM вызывает Task(), агент работает, Task() возвращает управление PM-у. PM продолжает цикл. PM не "ждёт" и не "завершает ход". Task() сам ждёт.

```
ЗАПРЕЩЕНО:
  ✗ Писать "ожидаю завершения агента" и остановиться
  ✗ Завершить свой ход/ответ до возврата Task()
...
ПРАВИЛЬНО:
  ...
  ✓ PM запускает следующий Task() — цикл продолжается непрерывно
```
...
| **Писать "ожидаю" и завершить ход** | **Task() блокирующий — PM ждёт внутри него** |
| **Останавливать цикл Фазы 3** | **Цикл непрерывный до 3.DONE** |
